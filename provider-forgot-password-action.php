...
if (isset($_POST['forgot'])) {
	extract($_POST);
	$sql = mysqli_query($con,"SELECT * from `service_provider_profile_tbl` where  `email` = '".$_POST['email']."' ");
	...
	if($nu > 0){
		...
		$url = "http://localhost:8000/Website/verifypassword.php?id=$email"; 
...
$mail->Username = "homeservicedetector@gmail.com";     
...
if($mail->send()){
	...
	echo "cde ma kai error che ";
		...
		header('location:provider-forgot-password.php');
	}
} 
?>